    ...
    public static class IdentityServiceExtension
    {
        public static IServiceCollection AddIdntityServices(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddAuthorization(opt => {
                opt.AddPolicy("IsActivyHost", policy => {
                    policy.Requirements.Add(new IHostRequirement());
                });
                opt.AddPolicy("IsEmailVerify", policy => {
                    policy.Requirements.Add(new IEmailVerificationRequirement());
                });
            });
            services.AddTransient<IAuthorizationHandler, IHostRequirementHandler>();
            services.AddTransient<IAuthorizationHandler, IEmailVerificationRequirementHandler>();
