...
export default observer(function ChatDashboard() {
  ...
  return (
    <div className="container grid-cols-3 grid h-screen">
      <div className="border bg-white overflow-y-scroll">
        <h1 className="p-4 text-2xl font font-bold border-b-2 bg-blue-700 text-white">
          Reactivities
        </h1>
        <div>
            ...
            >
              <div className="flex ml-4">
                ...
                <div className="ml-4">
                  ...
                  <p
                    className={`${
                      userStore.user?.userName !== e.fromUsername && !e.isRead
                        ? "font-bold"
                        : ""
                  ...
                  >
                    {e.file !== null &&
                      (e.fileType == "image/jpeg" ||
                        ...
                        e.fileType == "image/jpg") && <FaFileImage />}
                    {e.fileType && e.fileType == "audio/mpeg" && (
                      <FaFileAudio />
                    )}
                    {e.file !== null &&
                      (e.fileType == "video/mp4" ||
                        e.fileType == "video/mkv") && <FaFileVideo />}
                    ...
                    {userStore.user?.userName == e.fromUsername &&
                      (!e.isRead ? (
                        <HiMiniCheckCircle color="gray" />
                      ) : (
                        <HiMiniCheckCircle color="blue" />
                      ))}
                  </p>
                </div>
              </div>
      ...
      {conversationStore.laodignChat ? (
        ...
        <div className="border col-span-2 h-screen">
          <div className="bg-white flex items-center px-6 py-3 fixed w-full top-0">
            <img
              src={
                conversationStore.ProfileMessage?.image || "/assets/user.png"
              }
            ...
            <p className="text-lg font-semibold">
              ...
              <button
                onClick={() =>
                  conversationStore.connect(
                    conversationStore.ProfileMessage?.id!
                  )
                }
              >
                Call
                  ...
                  {files.map((e) => (
                    <div
                      className={` ml-2 inline-block border-2 rounded p-2 border-gray-500 ${
                        e.name == selectedFile?.name && "ring-2 ring-blue-500"
                      }`}
                    ...
                    >
                      ...
                      selectedFile?.contentType == "image/png" ||
                      selectedFile?.contentType == "image/jpg" ? (
                        <UploadFile File={e} className="w-[60px] h-[60px]" />
                      ) : (
                        <>
                          <UploadFile File={e} className="w-[60px] h-[60px]" />
                        </>
                        ...
                        {messages.map((x) =>
                          x.fromUsername == userStore.user?.userName ? (
                            <div className="flex justify-end mt-3 w-full items-center">
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    deleteMessage
                                      .mutateAsync(x.id!)
                                      .then(() =>
                                        conversationStore.deleteMessage(x.id!)
                                      )
                                  }
                                >
                                  Hapus
                              ...
                              <div className="px-2 py-1 bg-white max-w-80 rounded-l-lg rounded-br-lg ">
                                {x.file !== null && (
                                  <>
                                    <UploadFile
                                      File={x.file}
                                ...
                                <div className="flex items-center">
                                  ...
                                  <p className="text-gray-500 ml-2 flex text-xs font-semibold text-right mt-1 ">
                                    {format(new Date(x.createdAt), "hh:mm aa")}{" "}
                                    <HiMiniCheckCircle
                                      size={15}
              ...
              <Formik
                ...
                onSubmit={(values, { resetForm }) => {
                  ...
                  sendMessage
                    ...
                    .then((e) => {
                      const check = conversationStore.Conversation.some(
                        (e) => e.username === conversationStore.selectedUsername
                      );
                      if (!check) {
                        const Conversation: Conversation = {
                          displayName:
                            conversationStore.ProfileMessage?.displayName!,
                          username: conversationStore.ProfileMessage?.userName!,
