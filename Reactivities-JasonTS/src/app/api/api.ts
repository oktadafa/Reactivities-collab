...
export  const getActivities = async(params:string, startDate:string, PageNumber:number) => {
    const data = await axios.get<Pagination>(`/api/Activity?ListId=1&PageNumber=${PageNumber}&PageSize=2&startDate=${startDate}&params=${params}`);
    return data.data;
}

export  const getActivities2 = async({pageNumber}:{pageNumber:any}) => {
    const data = await axios.get<Pagination>(`/api/Activity?ListId=1&PageNumber=${pageNumber}&PageSize=2&startDate=2024-04-24T04%3A41%3A28.405Z`);
    return data.data;
}
...
export const getActivityByPredicate = async(username:string, predicate:string) : Promise<userActivity[]> => {
    const data = await axios.get(`/api/profiles/${username}/activities?predicate=${predicate}`)
    return data.data;
}
...
export const listFollowing = async(username:string, predicate:string) : Promise<Profile[]> => {
    const data= await axios.get(`/api/follow/${username}?predicate=${predicate}`)
    return data.data
}
